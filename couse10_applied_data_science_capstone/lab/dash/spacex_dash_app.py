...
def get_pie_chart(entered_site):
    df = spacex_df
    filtered_df_tmp = df[df['Launch Site']==str(entered_site)]
    filtered_df     = filtered_df_tmp.groupby('class')['Launch Site'].count().reset_index()
...
def get_scatter_chart(entered_site, selected_payload):
    # filtered_df_tmp = spacex_df[spacex_df['Launch Site']==str(entered_site)]
    # filtered_df     = filtered_df_tmp.groupby(['Launch Site', 'Payload Mass (kg)']).reset_index()
    if entered_site == 'ALL':
        filtered_df = spacex_df[(spacex_df['Payload Mass (kg)'] >= int(selected_payload[0])) &
                                (spacex_df['Payload Mass (kg)'] <= int(selected_payload[1]))
                               ]
